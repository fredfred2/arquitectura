...
public class PlayersResource {
    ...
    @PUT
    @Path("{id}")
    public Response createOrUpdatePlayer(@Context SecurityContext sCtx, @Context UriInfo uriInfo, @PathParam("id") String userName, Player player) {
        if (!isGroupPresent("players")) {
            createGroup("players", "Shared group for all Indian Rummy players");
        }
        ...
        if (isUserPresent(userName)) {
            if (sCtx.getUserPrincipal() == null) {
                ...
                return responseBuilder.build();
            } else if (!sCtx.isUserInRole("admin") && !sCtx.getUserPrincipal().getName().equals(userName)) {
